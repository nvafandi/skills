...
| [check-engineering-violations.sh](check-engineering-violations.sh) | Check for common engineering standard violations (Spring leftovers, primitive money, hardcoded values, inline query literals, manual collection loops, private member injection, `@Named` qualifiers, `System.out` direct printing) and output as JSON |
